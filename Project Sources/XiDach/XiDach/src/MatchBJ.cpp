...
void MatchBJ::splitTo(Player& p, int x) {
    p.splitHand(x);
...
void MatchBJ::InsurePlay(Player& p, Hand& h, Dealer& d) {
...
void MatchBJ::SplitPlay(Player& p, Hand& h, Dealer& d) {
    cout << "You splited your hand number " << h.Pos << " and drew 2 new cards for each" << '\n';
    splitTo(p, (h.Pos - 1));
    Match::PrintState(p, d);
    return;
}
void MatchBJ::SurrenderPlay(Player& p, Hand& h, Dealer& d) {
    cout << "You surrendered your hand and lost 0.5x bet" << '\n';
    h.surrendered = true;
}
void MatchBJ::DoublePlay(Player& p, Hand& h, Dealer& d) {
...
void MatchBJ::doResultBJ(Hand& p, Hand& d, int& result, StatBJ& stat) {
    if (result == 1) {
...
int MatchBJ::playerWantsBJ(Hand& p, Hand& d) { //Modify this to change the strategy of players. Note that dealer always stand on 17 or higher
...
bool MatchBJ::playerInsure(Hand& p, Hand& d) {
...
void MatchBJ::ProcessSimulationBJ(Dealer& dealer, StatBJ& stat) {
    //cout << "Dealer's first hand: ";
    //dealer.printprintDealerHand();
    Hand& d = dealer.hands[0];
    d.calculateScore();
    while (d.getScore() < 17) {
        dealCardToPlayer(dealer, 0);
...
void MatchBJ::SimulationBJ(ofstream& file) {
    ...
    for (int z = 0; z < u; z++) {
        ...
        ProcessSimulationBJ(dealer, stat);
    }
    cout << '\n';
    stat.setEVBJ();
    stat.printStat();
...
bool MatchBJ::DealerWantsBJ(Dealer& d) {
    d.hands[0].calculateScore();
    if (d.hands[0].getScore() < 17) return true;
    return false;
}

void MatchBJ::AnotherRound() {
...
int MatchBJ::GetSit(Player& p, Hand& h, Dealer& d) {
...
void MatchBJ::Options(Player& p, Hand& h, Dealer& d, int x) {
    char i;
    h.printHand();
    if (x == 0) {
        cout << "Buy (I)nsurance?";
    ...
    cout << "--> ";
    cin >> i;
    if (i == 'U') SurrenderPlay(p, h, d);
    else if (i == 'P') SplitPlay(p, h, d);
    else if (i == 'D') DoublePlay(p, h, d);
    else if (i == 'H') HitPlay(h);
    else if (i == 'S') StandPlay(p, h, d);
}

void MatchBJ::PrintState(Player& p, Dealer& d) {
    cout << "Current state: " << '\n';
    d.printDealerFirstHand();
}

void MatchBJ::AISuggest(Player& p, Hand& h, Dealer& d) {
...
void MatchBJ::ProcessPlayBJ(Dealer& d) {
    Stat stat; //
    d.printDealerFirstHand();
    players[0].printHand();
    ...
    for (auto& p : players) {
        ...
        for (auto& h : p.hands) {
            while (!h.surrendered && !h.stood && !h.isDoubled && !h.isBust) {
                int sit = GetSit(p, h, d);
                Options(p, h, d, sit);
            }
        }
    }

    while (DealerWantsBJ(d)) {
        d.printDealerHand();
        dealCardToHand(d.hands[0]);
